...
- `join`: 名前を送信して参加します。サーバーはプレイヤーIDと復帰トークンを返します。
- `resume`: スマートフォン更新後に、保存済みプレイヤーIDと復帰トークンで同じプレイヤーとして復帰します。
- `pedal`: `left` / `right` を交互に送信して自転車を進めます。
- `start-countdown`: 大画面の「カウント開始」ボタンから、管理PIN付きで少人数でもカウントを開始します。
- `reset`: 大画面の「リセット」ボタンから、管理PIN付きでゲームを初期化します。
...
大画面の `カウント開始` と `リセット` は管理PINで保護しています。Render などの公開環境では、環境変数 `ADMIN_PIN` に任意のPINを設定してください。

```text
ADMIN_PIN=任意のPIN
```

`ADMIN_PIN` が未設定の場合は `0000` が使われます。`ADMIN_PIN=0000` のときは管理PIN確認なしで、誰でも大画面から `カウント開始` と `リセット` を操作できます。公開イベントで管理操作を制限したい場合は必ず別の値に変更してください。

### セキュリティ対応

このゲームはイベント用の軽量Webアプリですが、公開URLで運用できるように最低限の保護を入れています。

- 管理操作の保護: `カウント開始` と `リセット` は `ADMIN_PIN` で保護します。`ADMIN_PIN=0000` の場合だけ開放モードになり、PIN確認なしで操作できます。
- 参加者操作の分離: 参加者はPINなしで `join` と `pedal` ができます。PINは運営用操作にだけ使います。
- 復帰トークン: スマートフォン更新後の復帰には、公開されるプレイヤーIDとは別に `resumeToken` を使います。`resumeToken` はスマートフォンの `localStorage` にだけ保存し、全体状態の `state` には含めません。
- DOM生成: 勝者名などのユーザー入力を画面に出す箇所は `textContent` を使います。`innerHTML` にユーザー入力を混ぜないことでXSSリスクを抑えます。
- セキュリティヘッダー: `helmet` でCSP、`X-Content-Type-Options`、`Referrer-Policy`、frame埋め込み制限などを設定します。
- 秘密情報管理: `ADMIN_PIN` はコードやREADMEに実値を書かず、RenderのEnvironment Variablesで設定してください。

注意点:

- ゲーム状態はメモリ上にあり、サーバー再起動で初期化されます。
- `ADMIN_PIN=0000` は便利な開放モードですが、公開イベントで勝手なリセットや開始を防ぎたい場合には向きません。
- 本格的な荒らし対策が必要な場合は、管理画面の分離、レート制限、参加用ルームコードなどを追加してください。
